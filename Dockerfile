...
RUN wget https://bootstrap.pypa.io/get-pip.py \
	&& python get-pip.py \
	&& wget https://raw.githubusercontent.com/espressif/esp-idf/master/requirements.txt \
...
# Get User / Group ID variables to create a build user/group in the image
ARG BUILD_UID
ARG BUILD_GID
RUN groupadd -g $BUILD_GID build \
	&& useradd -u $BUILD_UID -g $BUILD_GID -s /bin/bash -M -d /esp/project build
